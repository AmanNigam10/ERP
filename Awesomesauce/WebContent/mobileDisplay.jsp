...
$(document).ready(function(){
	...
	List<Product> prodList = prod_service.getProductsBySection("mobile");
	
	for(Product product : prodList)
	{
		String section = product.getSection();
		//System.out.println(product.getProdcutName()+product.getPrice());
...
function appendItem(pName, price, quantity, img){
	    	...
	    	'<div class="panel panel-primary text-center">' +
	    		...
	    		'<div class="panel-footer">' +
	    			'<div class="form-group">' +
	    				...
	    				'<input type="number" id="mobileQuantity' + listCount +'" min="1" name="mobileQuantity">' +
	    			'</div>' +
	...
	$("#mobileQuantity" + listCount).val(1);
...
function addToCartListener(name, listCount){
	...
	$("#addToCart"+listCount).click(function(){
		var pname = name;
		var quantity = $("#mobileQuantity" + listCount).val();
		...
		if(annyang){
			...
			var commands = {
				...
				'add to cart' : function(){
					...
					var quantity = $("#mobileQuantity" + focusFieldId).val();
					quantity = parseInt(quantity);
					$.get("addToCart.jsp?productName=" + pname+"&quantity="+quantity, function(data) {
